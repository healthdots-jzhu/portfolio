...
const Engagements = () => {
  ...
  const pageTitle = t('engagements.pageTitle');
  const impactItems = t('engagements.impactItems') || [];
  const merchImageAlts = t('engagements.merchImageAlts') || [];
  const merchImageSrcs = t('engagements.merchImageSrcs') || [];
  const thriftImageAlts = t('engagements.thriftImageAlts') || [];
  const thriftImageSrcs = t('engagements.thriftImageSrcs') || [];
  const leadershipBullets = t('engagements.leadershipBullets') || [];
  ...
  useEffect(() => {
    const cards = Array.isArray(t('engagements.cards')) ? t('engagements.cards') : [];
    const measure = () => {
      const next = {};
    ...
    <main className="engagements-page">
      ...
      <section className="engagements-cards">
        ...
        {Array.isArray(t('engagements.cards')) ? (
          ...
          <>
            <div className={`engagements-card ${expandedCards.about ? 'expanded' : ''}`}>
              <div className="engagements-card-icon">
                <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                  <path d="M170.5 73.581c0-11.647-4.569-22.596-12.866-30.831-17.127-17-44.992-16.999-62.119 0L50.623 87.306a3.959 3.959 0 0 0-1.172 2.809v54.463l-18.779 18.64a3.954 3.954 0 0 0-.001 5.618A4.001 4.001 0 0 0 33.5 170a4.004 4.004 0 0 0 2.828-1.163l18.78-18.64h54.873c1.062 0 2.081-.42 2.832-1.167l26.152-26.035c.314-.233.592-.508.825-.822l17.843-17.763c8.298-8.234 12.867-19.183 12.867-30.829zm-62.179 68.672H63.112l18.598-18.46h45.154l-18.543 18.46zm43.653-43.457l-17.129 17.053h-45.13l39.722-39.426a3.954 3.954 0 0 0 .001-5.618 4.02 4.02 0 0 0-5.657-.001l-66.328 65.835V91.761l43.72-43.393c14.008-13.905 36.801-13.905 50.807 0 6.786 6.734 10.522 15.688 10.522 25.212 0 9.523-3.736 18.477-10.522 25.211l-.006.005z" fill="#504E70"/>
                </svg>
              </div>
              <h3>{t('engagements.aboutHeading')}</h3>
              <p>{t('engagements.aboutBody')}</p>
              {!expandedCards.about && <button className="engagements-card-expand-btn" onClick={() => toggleExpand('about')}>Show More</button>}
              {expandedCards.about && <button className="engagements-card-expand-btn" onClick={() => toggleExpand('about')}>Show Less</button>}
            </div>
            ...
            <div className="engagements-card">
              ...
              <h3>{t('engagements.journeyHeading')}</h3>
              <p>{t('engagements.journeyBody')}</p>
            </div>
            ...
            <div className="engagements-card">
              ...
              <h3>{t('engagements.impactHeading')}</h3>
      ...
      <section className="merch-design">
        <h2>{t('engagements.merchHeading')}</h2>
        <p>{t('engagements.merchBody')}</p>
      ...
      <section className="creative-posts">
        <h2>{t('engagements.creativePostsHeading')}</h2>
        <p>{t('engagements.creativePostsIntro')}</p>
        <h3>{t('engagements.thriftHeading')}</h3>
        <p>{t('engagements.thriftStats')}</p>
      ...
      <section className="leadership">
        <h2>{t('engagements.leadershipHeading')}</h2>
        <ul>
      ...
      <section className="creative-reels">
        <h2>{t('engagements.creativeReelsHeading')}</h2>
        <p>{t('engagements.creativeReelsBody')}</p>
      </section>
    </main>
  );
};
